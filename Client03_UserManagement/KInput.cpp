...
KInput::InputState KInput::m_keysState[256] = { 0, };
...
void KInput::Update(double elapsedTime_)
{
    ...
    for (int i = 0; i < 256; ++i) {
        ...
        //if (m_keys[i] & 0x1) {
        //    if (m_keysState[i] == IDLE)
        //        m_keysState[i] = PRESSED;
        //    else if (m_keysState[i] == PRESSED)
        //        m_keysState[i] = STAY;
        //    else if (m_keysState[i] == RELEASED)
        //        m_keysState[i] = STAY;
        //}
        if (m_keys[i] & 0x8000) {
            if (m_keysState[i] == IDLE)
        ...
        {
            if (m_keysState[i] == PRESSED)
                m_keysState[i] = RELEASED;
            else if (m_keysState[i] == STAY)
                m_keysState[i] = RELEASED;
            else
                m_keysState[i] = IDLE;
        }
